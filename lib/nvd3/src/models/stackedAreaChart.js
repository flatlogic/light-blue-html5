...
nv.models.stackedAreaChart = function() {
    ...
    function chart(selection) {
        selection.each(function(data) {
            ...
            if (showLegend) {
                ...
                g.select('.nv-legendWrap')
                    .attr('transform', 'translate(' + ( availableWidth / 3 ) + ',' + (-margin.top) +')');
            }
            ...
            if (showAxes){
                ...
                yAxis
                    .scale(y)
                    .ticks(stacked.offset() == 'wiggle' ? 0 : availableHeight / 50)
    ...
    chart.showAxes = function(_) {
        if (!arguments.length) return showAxes;
        showAxes = _;
        if (!showAxes){
            //reset, cause no need for space for axes
